...
import study.querydsl.dto.MemberDto1;
import study.querydsl.dto.QMemberDto1;
...
public class QuerydslBasicTest {
	...
	//별칭이 다를 때, as를 사용하지 않으면 매칭이 되지 않아 값이 null 로 출력
	@Test
	...
	@Test
	public void findDtoByConstructor1() {
		QMember member = QMember.member;
		//getter setter
		List<UserDto> result = queryFactory
	...
	public void findDtoByQueryProjection() {
		QMember member = QMember.member;
		List<MemberDto1> result = queryFactory
				.select(new QMemberDto1(member.username, member.age))
				.from(member)
				.fetch();

		for(MemberDto1 memberDto : result) {
			System.out.println("memberDto = "+memberDto);
		}
	}
	...
	private Predicate ageEq1(Integer ageCond) {
		QMember member = QMember.member;
		if(ageCond == null) {
			return null;
		}
	...
	private BooleanExpression allEq(String usernameCond, Integer ageCond) {
		// chain 메소드 이므로 null 이 들어가면 오류 남.
	...
	public void bulkUpdate() {
		...
		//실행 후
		//벌크 연산을 한 후에는 영속성 컨택스트를 무시하고 바로 DB 에 저장하므로 조심해야 함
		// em.flush(); em.clear(); 해주는게 좋음
